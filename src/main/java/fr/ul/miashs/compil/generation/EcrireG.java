...
public class EcrireG {
    public StringBuilder generer_ecrire(Noeud ecrire) {
        StringBuilder res = new StringBuilder();
        ExpressionG expr = new ExpressionG();
        expr.generer_expression(ecrire.getFils().get(0), ecrire.getLabel()); // en generant l'exp (qui gère aussi le cas d'une constante ou d'un idf) on sait qu'il y a en haut de la pile le result à print
